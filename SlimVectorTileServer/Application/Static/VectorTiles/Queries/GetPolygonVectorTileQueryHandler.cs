...
{
    public class GetPolygonVectorTileQueryHandler : IRequestHandler<GetPolygonVectorTileQuery, byte[]>
    {
        ...
        public GetPolygonVectorTileQueryHandler(
        ...
        public async Task<byte[]> Handle(GetPolygonVectorTileQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                // Cache processing
                string cacheKey = $"polygon_tile_{request.Z}_{request.X}_{request.Y}_{request.UUID}";
                ...
                if (request.Z <= _cacheSettings.MaxCacheZoomLevel)
                {
                    ...
                    if (cachedTileData != null)
                    {
                        ...
                        if (_environment.IsDevelopment())
                        {
                            ...
                            debugMessage.AppendLine($"Polygon Tile \"z:{request.Z} x:{request.X} y:{request.Y}\" cache hit. total processing time: {stopwatch.ElapsedMilliseconds}ms");
                ...
                byte[] tileData = await _tilesService.CreatePolygonTileAsync(request.Z, request.X, request.Y, request.UUID);
